...
def train_PG(exp_name='',
    ...
    if discrete:
        ...
        sy_sampled_ac = categorical_sample_logits(sy_logits_na)  # Hint: Use the tf.multinomial op
        sy_logprob_n = -tf.nn.sparse_softmax_cross_entropy_with_logits(logits=sy_logits_na,
                                                                       labels=sy_ac_na)  # TODO: LOOK IF THIS IS CORRECT.

    else:
        sy_mean = build_mlp(sy_ob_no, ac_dim, scope="policy_mean")
    ...
    if nn_baseline:
        ...
        # YOUR_CODE_HERE
        sy_baseline_loss = tf.reduce_mean(tf.square(baseline_prediction - sy_adv_n))
        baseline_update_op = tf.train.AdamOptimizer(learning_rate).minimize(sy_baseline_loss)
    ...
    for itr in range(n_iter):
        ...
        if reward_to_go:
            ...
            for path in paths:
                R = 0
                for (k, r) in enumerate(path['reward']):
                    R += gamma ** k * r
                q_n += [R] * pathlength(path)
            q_n = np.array(q_n)
        ...
        if nn_baseline:
            ...
            # YOUR_CODE_HERE
            q_n = (q_n - np.mean(q_n)) / (np.std(q_n) + 1e-6)
            b_loss, _ = sess.run([sy_baseline_loss, baseline_update_op], feed_dict={sy_ob_no: ob_no, sy_adv_n: q_n})
        ...
        _ = sess.run(update_op, feed_dict={sy_ob_no: ob_no, sy_ac_na: ac_na, sy_adv_n: adv_n})
